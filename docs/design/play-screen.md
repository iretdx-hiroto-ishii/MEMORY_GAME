...
| 区分 | 内容 |
|------|------|
| **Must（維持・文書化）** | 4×4 カードグリッド / めくり・一致判定 / ポイント・コンボ / ヘッダー / ステータスバー / フッター操作（メニュー・リザルト） / カウントダウンオーバーレイ / 5秒プレビュー / メニュー・確認・リザルト・遊び方・設定モーダル |
...
| **Won't（今回やらない）** | localStorage スコア履歴 / 制限時間 / 操作エフェクト強化 / 難易度選択 |
...
- メニュー経由で遊び方・設定・やり直し・タイトル戻り、およびリザルト確認を **画面内完結**で行える。
...
| プレイ画面 | メニュー「やり直し」→確認OK | 同一画面内でカウントダウン→プレビュー→再プレイ |
| プレイ画面 | 全カードクリア | リザルト overlay 自動表示（1秒後） |
| プレイ画面 | 🏁（クリア後のみ有効） | リザルト overlay 手動表示 |
| プレイ画面 | ⋯ | メニュー overlay |
| メニュー overlay | 遊び方 / 設定 | 各 overlay を表示 |
| メニュー overlay | タイトルに戻る / やり直し | 確認 overlay を表示 |
...
| `btn-result` | フッター | 🏁 | `openResult`（`isFinished` 時のみ有効） | `App.tsx` |
...
| 表示内容 | `MAX COMBO` / `POINT`（項目名左寄せ・値右寄せ） |
| 操作 | `リトライ` / `タイトルに戻る` ボタンを表示 |
| `record-box` | **空**（スコア履歴 Won't） |
| 閉じ方 | ❎ → `closeResult` |
...
| 項目順 | `やり直し` → `遊び方` → `設定` → `タイトルに戻る` |
| 操作 | 遊び方/設定を開く、やり直し/タイトル戻りは確認ダイアログを挟む |
...
- `isFinished === false` のとき `disabled` クラス（操作不可）

#### メニュー経由の再プレイ / タイトル戻り

- メニューの `やり直し` / `タイトルに戻る` は確認ダイアログを表示。
...
- [ ] AC-3: メニュー `やり直し`（確認OK）またはリザルト `リトライ` で `ready...` → 3,2,1 → 5秒プレビュー → プレイ可能になる
- [ ] AC-4: クリア後、リザルトに POINT / MAX COMBO が表示される
- [ ] AC-5: ⋯ でメニュー overlay が開き、`やり直し / 遊び方 / 設定 / タイトルに戻る` が表示される
- [ ] AC-6: メニューの `やり直し` と `タイトルに戻る` は確認 overlay を経由して実行される
- [ ] AC-7: リザルト overlay に `リトライ` / `タイトルに戻る` ボタンが表示され、各動作が実行される
- [ ] AC-8: 遊び方ダイアログ本文が白文字で表示される
- [ ] AC-9: クリア前は 🏁 が無効、クリア後は有効
- [ ] AC-10: フッターに `AppFooter` によるコピーライトが表示される

### タイトル画面連携（新規）

- [ ] AC-11: タイトル「スタート」後のみプレイ画面が表示され、AC-3 の開始シーケンスが走る
- [ ] AC-12: アプリ起動直後にプレイ画面が **単独で** 表示されない（タイトル経由）

### 収益向け（推奨）

- [ ] AC-13: フッター下に広告プレースホルダ枠があり、カード・ボタンと重ならない
