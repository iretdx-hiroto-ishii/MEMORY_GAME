...
| 作成日 | `2026-05-31` |
| 更新日 | `2026-05-31` |
| 関連ブランチ | `main` / `feature/add-title-page` |
...
| 区分 | 内容 |
|------|------|
| **Must（維持・文書化）** | 4×4 カードグリッド / めくり・一致判定 / ポイント・コンボ / ヘッダー / ステータスバー / フッター操作（ルール・リセット・リザルト） / カウントダウンオーバーレイ / 5秒プレビュー / リザルト・ルールモーダル |
...
| **Won't（今回やらない）** | localStorage スコア履歴 / 制限時間 / 操作エフェクト強化 / 難易度選択 / プレイ中の設定ボタン / タイトルへ戻るボタン |
...
| プレイ画面 | 🔁 | 同一画面内でカウントダウン→プレビュー→再プレイ |
...
※ countdown / result / rules は shadow-overlay で全画面覆い
```
...
| `btn-rules` | フッター | 📋 | `openRules` | `App.tsx` |
| `btn-reset` | フッター | 🔁 | `resetGame` | `App.tsx` |
...
| 表示内容 | `POINT: {point}` / `MAX COMBO: {maxCombo}` |
...
- [ ] AC-3: 🔁 で `ready...` → 3,2,1 → 5秒プレビュー → プレイ可能になる
- [ ] AC-4: クリア後、リザルトに POINT / MAX COMBO が表示される
- [ ] AC-5: 📋 でルール overlay が開き、❎ で閉じる
- [ ] AC-6: クリア前は 🏁 が無効、クリア後は有効
- [ ] AC-6b: フッターに `AppFooter` によるコピーライトが表示される

### タイトル画面連携（新規）

- [ ] AC-7: タイトル「スタート」後のみプレイ画面が表示され、AC-3 の開始シーケンスが走る
- [ ] AC-8: アプリ起動直後にプレイ画面が **単独で** 表示されない（タイトル経由）

### 収益向け（推奨）

- [ ] AC-9: フッター下に広告プレースホルダ枠があり、カード・ボタンと重ならない
...
| 版 | 日付 | 変更内容 |
|----|------|----------|
| 0.1.0 | 2026-05-31 | 現行 `App.tsx` 実装の文書化。MVP（タイトル連携・広告枠）差分を追記 |
